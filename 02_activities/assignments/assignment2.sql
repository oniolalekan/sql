...
SELECT 
product_name || ', ' || COALESCE(product_size, ' ') || ' (' || COALESCE(product_qty_type, 'unit') || ')' AS "Full Product Description"
FROM product;
...
-- Get the highest and lowest sales dates
...
WITH VendorProductRevenue AS (
    SELECT 
        v.vendor_name,
        p.product_name,
        c.customer_id,
        vi.original_price,  -- Assuming the product price is available in the `vendor_inventory` table
        5 * vi.original_price AS revenue_per_customer
    FROM vendor_inventory vi
    JOIN vendor v ON vi.vendor_id = v.vendor_id
    JOIN product p ON vi.product_id = p.product_id
    JOIN customer c ON 1 = 1  -- Cross join with customers
)
SELECT 
    vendor_name,
    product_name,
